...
//FFT Configuration:
uint32_t sampleSize = 8000;
//arduinoFFT fft = arduinoFFT();
double inputSignal[100];
double magnitudes[50];
...
void fftInit(){
  Serial.println("ftt Initialisation: ");
  Serial.print("size: ");
  Serial.println(i);

  sampleSize = i;
  double newInputSignal[sampleSize];
  memcpy(newInputSignal, inputSignal, sizeof(inputSignal));  
  memcpy(inputSignal, newInputSignal, sizeof(newInputSignal));  

  double newMagnitudes[sampleSize/2];
  memcpy(newMagnitudes, inputSignal, sizeof(inputSignal)); 
  memcpy(inputSignal, newMagnitudes, sizeof(newMagnitudes));

  double size = (double) sampleSize;
  
  //fft.Compute(inputSignal, magnitudes, size,  FFT_FORWARD);
}
