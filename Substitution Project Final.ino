...
#define RIF10_F32_TAP_NUM 23
#define RIF15_F32_TAP_NUM 5
#define RIF20_F32_TAP_NUM 0
#define RIF10_F44_TAP_NUM 31
#define RIF15_F44_TAP_NUM 37
#define RIF20_F44_TAP_NUM 5
...
Adafruit_SSD1306 display1(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, OLED_RESET); //We create the display entity
...
static const int8_t rif10_f44_taps[RIF10_F44_TAP_NUM] = {
  -4,
  -11,
  -2,
  5,
...
static const int8_t rif15_f44_taps[RIF15_F44_TAP_NUM] = {
...
static const int8_t rif20_44_taps[RIF20_F44_TAP_NUM] = {
  -52,
  4,
  124,
  4,
          ...
          default:
            {
              ...
              for (int coef = 0; coef <= RIF10_F44_TAP_NUM; coef++) {
                outputValue += rif10_f44_taps[coef] * adcBuffer[RIF10_F44_TAP_NUM - coef];
              }
              /*
          ...
          default:
            {
              ...
              for (int coef = 0; coef <= RIF15_F44_TAP_NUM; coef++) {
                outputValue += rif15_f44_taps[coef] * adcBuffer[RIF15_F44_TAP_NUM - coef];
              }
              /*
              //BY default, we use the F44 FilterFrequency:
...
void drawFields() {
  ...
  display1.fillRect(borderWidth, borderWidth, (display1.width() / 2) - 2 * borderWidth, (display1.height() / 3) - 2 * borderWidth + 1, BLACK);

  display1.drawRect(display1.width() / 2, 0, display1.width() / 2, display1.height() / 3, WHITE);
  display1.fillRect((display1.width() / 2) + borderWidth, borderWidth, (display1.width() / 2) - 2 * borderWidth, (display1.height() / 3) - 2 * borderWidth + 1, BLACK);

  display1.drawRect(0, (display1.height() / 3), display1.width(), display1.height() / 1.4, WHITE);
  display1.fillRect(borderWidth, (display1.height() / 3) + borderWidth, (display1.width()) - 2 * borderWidth, (display1.height() / 1.5) - 2 * borderWidth, WHITE);
}
...
void loop() {
  ...
  //Filter use: (depends if it is on)
